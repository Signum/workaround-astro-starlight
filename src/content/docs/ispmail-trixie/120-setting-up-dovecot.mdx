...
slug: ispmail-trixie/setting-up-dovecot
sidebar:
  order: 120
---
...
system users (those from /etc/passwd). But we want to use the SQLite database backend so go ahead and change this block
to:
...
(TODO: explain that mail_location is deprecated)
...
And enforce TLS encryption for the entire connection by setting:
...
- userdb: where to find a user’s mailbox in the file system (TODO: nur für quota relevant)
...
```
sql_driver = sqlite
sqlite /root/mailserver-demo.sqlite {}
```
